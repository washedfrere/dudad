...
' : '').'
'.(($cx['funcs']['ifvar']($cx, ((isset($in['new_replies']) && is_array($in)) ? $in['new_replies'] : null))) ? '                <a title="'.$cx['funcs']['ch']($cx, 'i18n', array(array('new replies'),array()), 'raw').'" href="'.$cx['funcs']['ch']($cx, 'const', array(array('RURI'),array()), 'raw').'topic/'.((isset($in['topic_id']) && is_array($in)) ? $in['topic_id'] : null).'/'.htmlentities((string)((isset($in['safe_title']) && is_array($in)) ? $in['safe_title'] : null), ENT_QUOTES, 'UTF-8').'/post-'.((isset($in['first_new_reply']) && is_array($in)) ? $in['first_new_reply'] : null).'#post-'.((isset($in['first_new_reply']) && is_array($in)) ? $in['first_new_reply'] : null).'">
                    <div class=\'codo_unread_replies\'>'.((isset($in['new_replies']) && is_array($in)) ? $in['new_replies'] : null).'</div>
                </a>
' : '').'
            </div>

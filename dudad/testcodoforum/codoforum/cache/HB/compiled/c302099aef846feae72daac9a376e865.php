...
'.$cx['funcs']['sec']($cx, ((isset($in['topics']) && is_array($in)) ? $in['topics'] : null), $in, true, function($cx, $in) {return '    <article class="clearfix">
...
' : '').'            </div>
        ...
        <div class="codo_topics_topic_foot codo_topics_topic_moderation clearfix">

            <input type="checkbox">
            <div data-what="'.htmlentities((string)((isset($in['what']) && is_array($in)) ? $in['what'] : null), ENT_QUOTES, 'UTF-8').'" data-tid="'.htmlentities((string)((isset($in['topic_id']) && is_array($in)) ? $in['topic_id'] : null), ENT_QUOTES, 'UTF-8').'" data-pid="'.htmlentities((string)((isset($in['post_id']) && is_array($in)) ? $in['post_id'] : null), ENT_QUOTES, 'UTF-8').'" class="codo_topics_topic_moderation_action">
                <button class="codo_btn codo_btn_primary codo_btn_approve">'.$cx['funcs']['ch']($cx, 'i18n', array(array('Approve'),array()), 'encq').'</button>
                <button class="codo_btn codo_btn_danger codo_btn_delete">'.$cx['funcs']['ch']($cx, 'i18n', array(array('Delete'),array()), 'encq').'</button>
            </div>
        </div>
    </article>
';}).'</div>';
